(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.$formSetMonto = this.$element.find( "#webpayForm" );
			this.$formSetMonto2 = this.$element.find( "#formInsertOrder" );
			this.$formAddToCartNotLogged = this.$element.find( "form.add-to-cart-notLogged" );
			this.$formCart = this.$element.find( "#shopping-cart" ); // Shopping cart form
			this.$setCantProducts = this.$element.find( "form.formInsertOrder" ); // Shopping cart form
		...
		setMontoFormInsert: function() {
			var self = this;
			if( self.$formSetMonto2.length ) {
				...
				if( items.length > 0 ) {				
					for( var i = 0; i < items.length; ++i ) {
						...
						var qty = item.qty;
						html += ("/" + product + "/" + price + "/" + qty + "#");
					}

				}
				
				var $form = self.$formSetMonto2;
				$form.find( "input[name='monto']" ).val(total);
				$form.find( "input[name='cantProducts']" ).val(html);
			}
		},

		setMontoWebpay: function() {
			var self = this;
			if( self.$formSetMonto.length ) {
				var total = this.storage.getItem( this.total );
				
				var $form = self.$formSetMonto;
				$form.find( "input[name='monto']" ).val(total);
			}
		},
		...
		handleAddToCartForm: function() {
			...
			self.$formAddToCartNotLogged.each(function() {
				var $form = $( this );
